...
purge_live_settings()
{
FBSDFLAVOUR=$(cat /usr/local/etc/default/distro | grep FLAVOUR | cut -d = -f2)
pkg delete -y $FBSDFLAVOUR-live-settings
pkg delete -y freebsd-live-common-settings
}

clean_root_and_auto_login()
{
sed -i "" -e 's/root/Pc/g' /etc/ttys
rm -rf /root/cardDetect /root/functions.sh /root/sysconfig.sh /root/sysutil.sh /root/sysutil.sh /root/.login /root/Desktop/fbi.desktop
...
revert_slim()
{
  # remove freebsd user from slim.conf and autologin
...
if [ -f /usr/local/share/config/kdm/kdmrc ]; then
     ...
     -e  "s/AutoLoginUser=freebsd/#AutoLoginUser=fred/g" \
...
if [ -d /usr/local/share/freebsd/kde-settings ]; then
...
# Replassing FreeBSD by FreeBSD
sed -i '' 's/OS="FreeBSD"/OS="FreeBSD"/g' /usr/local/etc/grub.d/10_kfreebsd

# Removing livecd hostname.
( echo 'g/hostname="livecd"/d' ; echo 'wq' ) | ex -s /etc/rc.conf

rm -f /usr/local/etc/xdg/autostart/umountfreebsd.desktop
